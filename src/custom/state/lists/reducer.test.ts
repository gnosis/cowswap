...
import { DEFAULT_LIST_OF_LISTS, getDefaultListByChainId } from 'constants/lists'
import { updateVersion } from '@src/state/global/actions'
import { fetchTokenList, acceptListUpdate, addList, removeList, selectList } from 'state/lists/actions'
...
function wrapStateInChainId(chainId: ChainId, data: ListsState[ChainId]) {
  return {
    [chainId]: data
  }
}
...
describe('list reducer', () => {
  const SELECTED_CHAIN_ID = ChainId.MAINNET
  const DEFAULT_TOKEN_LIST_URL = getDefaultListByChainId(SELECTED_CHAIN_ID)
  ...
  beforeEach(() => {
    store = createStore(
      reducer,
      wrapStateInChainId(SELECTED_CHAIN_ID, {
        byUrl: {},
        selectedListUrl: undefined,
        lastInitializedDefaultListOfLists: undefined
      })
    )
  })
  ...
  describe('fetchTokenList', () => {
    describe('pending', () => {
      it('sets pending', () => {
        store.dispatch(fetchTokenList.pending({ requestId: 'request-id', url: 'fake-url', chainId: SELECTED_CHAIN_ID }))
        expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
          byUrl: {
            'fake-url': {
              error: null,
  ...
  describe('addList', () => {
    it('adds the list key to byUrl', () => {
      store.dispatch(addList({ url: 'list-id', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {
          'list-id': {
            error: null,
    ...
    it('no op for existing list', () => {
      ...
      store.dispatch(addList({ url: 'fake-url', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {
          'fake-url': {
            error: null,
  ...
  describe('acceptListUpdate', () => {
    it('swaps pending update into current', () => {
      ...
      store.dispatch(acceptListUpdate({ url: 'fake-url', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {
          'fake-url': {
            error: null,
  ...
  describe('removeList', () => {
    it('deletes the list key', () => {
      ...
      store.dispatch(removeList({ url: 'fake-url', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {},
        selectedListUrl: undefined
      })
  ...
  describe('selectList', () => {
    it('sets the selected list url', () => {
      ...
      store.dispatch(selectList({ url: 'fake-url', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {
          'fake-url': {
            error: null,
    ...
    it('selects if not present already', () => {
      ...
      store.dispatch(selectList({ url: 'fake-url-invalid', chainId: SELECTED_CHAIN_ID }))
      expect(store.getState()[SELECTED_CHAIN_ID]).toEqual({
        byUrl: {
          'fake-url': {
            error: null,
  ...
  describe('updateVersion', () => {
    describe('never initialized', () => {
      ...
      it('clears the current lists', () => {
        expect(
          store.getState()[SELECTED_CHAIN_ID].byUrl[
            'https://unpkg.com/@uniswap/default-token-list@latest/uniswap-default.tokenlist.json'
          ]
        ).toBeUndefined()
        expect(
          store.getState()[SELECTED_CHAIN_ID].byUrl['https://unpkg.com/@uniswap/default-token-list@latest']
        ).toBeUndefined()
      })

      it('puts in all the new lists', () => {
        expect(Object.keys(store.getState()[SELECTED_CHAIN_ID].byUrl)).toEqual(DEFAULT_LIST_OF_LISTS[SELECTED_CHAIN_ID])
      })
      it('all lists are empty', () => {
        const s = store.getState()[SELECTED_CHAIN_ID]
      ...
      it('sets initialized lists', () => {
        expect(store.getState()[SELECTED_CHAIN_ID].lastInitializedDefaultListOfLists).toEqual(
          DEFAULT_LIST_OF_LISTS[SELECTED_CHAIN_ID]
        )
      })
      it('sets selected list', () => {
        store.dispatch(selectList({ url: 'tokens.uniswap.eth', chainId: SELECTED_CHAIN_ID }))
        expect(store.getState()[SELECTED_CHAIN_ID].selectedListUrl).toEqual(getDefaultListByChainId(SELECTED_CHAIN_ID))
      })
      it('default list is initialized', () => {
        expect(store.getState()[SELECTED_CHAIN_ID].byUrl[DEFAULT_TOKEN_LIST_URL]).toEqual({
          error: null,
          current: null,
          loadingRequestId: null,
          pendingUpdate: null
    ...
    describe('initialized with a different set of lists', () => {
      ...
      it('adds all the lists in the default list of lists', () => {
        expect(Object.keys(store.getState()[SELECTED_CHAIN_ID].byUrl)).toContain(DEFAULT_TOKEN_LIST_URL)
      })

      it('each of those initialized lists is empty', () => {
        const byUrl = store.getState()[SELECTED_CHAIN_ID].byUrl
      ...
      it('sets default list to selected list', () => {
        // is undefined in beginning, stays undefined
        expect(store.getState()[SELECTED_CHAIN_ID].selectedListUrl).toEqual(undefined)
      })
      it('default list is initialized', () => {
        expect(store.getState()[SELECTED_CHAIN_ID].byUrl[DEFAULT_TOKEN_LIST_URL]).toEqual({
          error: null,
