...
const generateOrder = ({ owner, sellToken, buyToken }: GenerateOrderParams): Order => {
  ...
  const kindName = !!kind ? 'BUY' : 'SELL'

  const summary = `Order ${kindName.toUpperCase()} ${(sellAmount / 10 ** sellToken.decimals).toFixed(2)} ${
    sellToken.symbol
...
export const useAddOrdersOnMount = (minPendingOrders = 5) => {
