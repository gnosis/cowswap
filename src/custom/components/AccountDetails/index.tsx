...
import Copy from '@src/components/AccountDetails/Copy'
import Transaction from '@src/components/AccountDetails/Transaction'

import { SUPPORTED_WALLETS } from '../../constants'
import { ReactComponent as Close } from '@src/assets/images/x.svg'
...
function renderTransactions(transactions: string[], title?: React.ReactNode) {
  return (
    <>
      {title && <small>{title}</small>}
      <TransactionListWrapper as="ul">
...
function renderOrders(pendingOrders: Order[], title?: React.ReactNode) {
  return (
    <>
      {title && <small style={{ textDecoration: 'underline' }}>{title}</small>}
      <TransactionListWrapper as="ul">
        {pendingOrders.map((order, i) => {
          return <Transaction key={i} hash={order.id} />
        })}
      </TransactionListWrapper>
    </>
  )
...
export default function AccountDetails({
  ...
  return (
    <>
      ...
      {!!pendingOrders.length || !!pendingTransactions.length || !!confirmedTransactions.length ? (
        <LowerSection>
          <AutoRow mb={'1rem'} style={{ justifyContent: 'space-between' }}>
            <TYPE.body>Recent Activity</TYPE.body>
            <LinkStyledButton onClick={clearAllTransactionsCallback}>(clear all)</LinkStyledButton>
          </AutoRow>
          {!!pendingOrders.length && renderOrders(pendingOrders, 'Pending Orders')}
          {!!pendingTransactions.length && renderTransactions(pendingTransactions, 'Pending Transactions')}
          {!!confirmedTransactions.length && renderTransactions(confirmedTransactions, 'Confirmed Transactions')}
        </LowerSection>
      ) : (
        <LowerSection>
